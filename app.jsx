...
function ResultsTable({ selectedColor, matches, totalProducts, pinnedItems, togglePin, wishlist, toggleWishlist, toneRamp, toneIdx, setToneIdx }) {
  ...
  function toggleFormat(f) {
    if (!activeFormats.includes(f)) window.gtag?.('event', 'apply_filter', { filter_type: 'format', filter_value: f });
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', minHeight:0 }}>
      ...
      {/* Compact filter bar */}
      {(orderedTones.length >= 1 || allBrands.length > 1 || allTiers.length > 1 || allFormats.length > 1 || hasDiscontinued) && (
        <div style={{ display:'flex', flexWrap:'wrap', alignItems:'center', gap:8, marginBottom:14 }}>
          ...
          {allFormats.length > 1 && (
            <FilterDropdown label="Format" count={activeFormats.length}
              isOpen={openFilter==='Format'} onOpen={setOpenFilter}
              ...
              {allFormats.map(f => {
                ...
                return (
                  <button key={f} onClick={() => toggleFormat(f)} style={{
                    ...
                    cursor:'pointer', fontFamily:'DM Sans', fontWeight: active ? 500 : 400,
                    letterSpacing:'0.04em', transition:'all 0.15s', whiteSpace:'nowrap',
                  }}>{FORMAT_LABELS[f] || f}{active && <span style={{ marginLeft:5, opacity:0.6, fontSize:10 }}>✕</span>}</button>
                );
              })}
            </FilterDropdown>
          )}
          ...
          {allFinishes.length > 1 && (
            <FilterDropdown label="Finish" count={activeFinishes.length}
              ...
              {allFinishes.includes('Unlisted') && (
                <p style={{
                  width:'100%', margin:'0 0 2px', fontFamily:'Cormorant Garamond', fontStyle:'italic',
                  fontSize:14, color:'var(--text-muted)', lineHeight:1.35,
                }}>
                  Finish isn't listed for every product — "Unlisted" includes those.
                </p>
              )}
              {allFinishes.map(f => {
          ...
          {hasDiscontinued && (
            <button onClick={() => {
              ...
              cursor:'pointer', fontFamily:'DM Sans', fontWeight: hideDiscontinued ? 500 : 400,
              letterSpacing:'0.04em', transition:'all 0.15s', whiteSpace:'nowrap',
            }}>Still Made</button>
          )}

          {(activeBrands.length > 0 || activeTones.length > 0 || activeTiers.length > 0 || activeFinishes.length > 0 || activeFormats.length > 0) && (
...
function drawShareImage(canvas, wishlist, images) {
  ...
  for (let i = 0; i < wishlist.length; i++) {
    ...
    if (photoImg) {
      ...
      const dw = photoImg.naturalWidth * s, dh = photoImg.naturalHeight * s;
  ...
  ctx.strokeStyle = '#E0D0C4';
  ctx.beginPath();
  ctx.moveTo(80, H - 118); ctx.lineTo(W - 80, H - 118);
  ctx.stroke();
      ...
      <div style={{
        ...
        <div style={{
          ...
          <p style={{ fontSize:13, color:'var(--text-muted)', fontFamily:'DM Sans', lineHeight:1.5, marginTop:-4 }}>
            A portrait card sized for Instagram (4:5), with each lipstick photo, its shade, and a link back to {SITE_URL}. Save it and post it to your story, feed, or send it to a friend.
          </p>
...
function parseSharedLink(search) {
  ...
  const keys = items.map(it => {
    const i = it.indexOf('|');
    return i > 0 ? { brand: it.slice(0, i), shade: it.slice(i + 1) } : null;
  }).filter(Boolean);
  if (keys.length) return { type:'list', keys };
  return null;
}

function findProduct(brand, shade) {
  const b = brand.toLowerCase(), s = shade.toLowerCase();
...
function SharedListPanel({ items, missing, loading, wishlist, toggleWishlist, onSaveAll, onClose }) {
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {items && (
            <span style={{ fontSize:11, color:'var(--text-muted)', letterSpacing:'0.06em' }}>
              {items.length} {items.length === 1 ? 'shade' : 'shades'}
            </span>
          )}
          <button onClick={onClose} style={{
            marginLeft:'auto', width:32, height:32, borderRadius:'50%',
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'20px 32px' }}>
          {loading ? <CatalogueLoading /> : (
            <>
              <p style={{ fontFamily:'Cormorant Garamond', fontStyle:'italic', fontSize:16, color:'var(--text-muted)', marginBottom:14 }}>
                Someone shared these shades with you. Tap a heart to save one to your favorites.
              </p>
              <ul style={{ listStyle:'none', display:'flex', flexDirection:'column', gap:10 }}>
                {items.map(p => {
              ...
              {missing > 0 && (
                <p style={{ fontSize:11, color:'var(--text-muted)', marginTop:14, fontFamily:'DM Sans' }}>
                  {missing} {missing === 1 ? 'shade is' : 'shades are'} no longer in the catalogue.
                </p>
              )}
            </>
          )}
        ...
        {!loading && items.length > 0 && (
          <div style={{ borderTop:'1px solid var(--border)', padding:'20px 32px 24px', background:'#fff' }}>
            <button onClick={onSaveAll} disabled={!unsaved.length} style={{
    ...
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <Section title="Finish">
          <p style={{
            margin:'0 0 10px', fontFamily:'Cormorant Garamond', fontStyle:'italic',
            fontSize:14, color:'var(--text-muted)', lineHeight:1.35,
          }}>
            Finish isn't listed for every product — pick "Unlisted" to still include those.
          </p>
...
function DupeFinder({ product, onSelect, onUsePhoto }) {
  ...
  useEffect(() => { if (product) setBrand(product.brand); }, [product]);
...
function App() {
  ...
  const sharedList = React.useMemo(() => {
    ...
    for (const k of sharedListKeys) {
      ...
      if (p && !items.some(q => q.brand === p.brand && q.shade === p.shade)) {
        items.push({ brand: p.brand, product: p.product, shade: p.shade, finish: p.finish, hex: p.hex });
      }
    }
    return { items, missing: sharedListKeys.length - items.length };
  ...
  function saveSharedList() {
    ...
    setWishlist(prev => {
      ...
      window.gtag?.('event', 'save_shared_list', { item_count: add.length });
      return [...prev, ...add];
    });
  }
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', flexDirection:'column', position:'relative', zIndex:1 }}>
      ...
      {/* Shared list (opened from a link) */}
      {sharedListKeys && (
        <SharedListPanel
